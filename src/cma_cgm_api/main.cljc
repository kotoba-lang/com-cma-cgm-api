(ns cma-cgm-api.main
  "Kotodama WASM entrypoint for the CMA CGM API clean-room actor (L4) — Clojure port.
  ...
  facts (`cma_cgm_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cma_cgm_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "cmacgmap_tra"
    :fields [:gtin :description :brand]        :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"      :plural "events"      :id-prefix "cmacgmap_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"   :plural "documents"   :id-prefix "cmacgmap_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container"  :plural "containers"  :id-prefix "cmacgmap_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"      :plural "parties"     :id-prefix "cmacgmap_par"
    :fields [:gln :name :role]                 :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "cmacgmap_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"cma_cgm_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cma_cgm_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cma_cgm_api-compat/ready)
