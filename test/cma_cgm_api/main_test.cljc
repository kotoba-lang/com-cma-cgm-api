(ns cma-cgm-api.main-test
  "Contract + behavioral test for the cma_cgm_api-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "cma_cgm_api-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'cma-cgm-api.main-test)]
